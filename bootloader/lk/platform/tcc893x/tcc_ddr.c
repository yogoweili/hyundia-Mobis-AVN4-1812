...
#define sdram_test_addr(x) (*(volatile unsigned long *)(0x80000000 + (x)))
...
{
#ifndef DRAM_OVERCLOCK_TEST
	{0x41019006, 0x00200011, 8000000},
	{0x41018606, 0x00200011, 7800000},
	{0x41018106, 0x00200011, 7700000},
	{0x41017C06, 0x00200011, 7600000},
	{0x41017706, 0x00200011, 7500000},
	{0x41017206, 0x00200011, 7400000},
...
* @date 2013/07/04
*
* telechips email notice patch for eMMC boot
**/
    	*(volatile unsigned long *)addr_clk(0x0000D8) = 0x2000001A;  // sdmmc1 peri (24MHz) 	// for emmcboot
...
#ifdef DDR3_TIMING_PATCH
	ddr_phy(0xd) = 28;	//delay @ SK Hynix
#endif
	#ifdef ZDEN
...
volatile void InitRoutine_Start2(void)
{
...
// Not Used
	...
	unsigned int dummy_address1;
	unsigned int dummy_address2;
#endif
...
//	ddr_phy(0x03) = ddr_phy(0x03) | ( 0x3 << 11 ) | ( 0x3 << 13 );
	...
	dummy_address1 = 0x80004000;
	dummy_address2 = 0x80008000;
	
	for(i=0 ; i<8 ; i++){
		sdram_test_addr(0x4000+i*4) = 0x1234abcd;
		sdram_test_addr(0x8000+i*4) = 0xabcd1234;
	}
	
	__asm__ volatile("mov   r1, %0"::"r"(test_address));
	__asm__ volatile("mov   r2, %0"::"r"(dummy_address1));
	__asm__ volatile("mov   r3, %0"::"r"(dummy_address2));
	{
		//GPIO F16
		//base address : 0x74200000;
//		*((unsigned int *)(0x74200178)) = 0x0; //GPFFNC2
//		*((unsigned int *)(0x74200144)) = 0x00010000; //GPFEN

	   	while(1){
//			*((unsigned int *)(0x74200140)) = 0x00010000; //GPFDAT
			__asm__ volatile("ldmia r2, {r4-r11}");
	    	__asm__ volatile("stmia r1, {r4-r11}");
	
//			*((unsigned int *)(0x74200140)) = 0x00000000; //GPFDAT
			__asm__ volatile("ldmia r3, {r4-r11}");
	    	__asm__ volatile("stmia r1, {r4-r11}");
	   	} 
	}	
#endif
...
volatile void InitRoutine_End2(void)
{
...
static unsigned int get_membus_ckc(unsigned int mem_freq)
{
	...
	if(mem_freq > 6000000){
		...
		CKC_CHANGE_ARG(PLL_VALUE) = vsel<<30|s<<24|m<<8|p<<0;
		CKC_CHANGE_ARG(CKC_CTRL_VALUE) = 0x00200011;
		CKC_CHANGE_ARG(CLK_RATE) = mem_freq/10000;
...
//	Address 0x00000000 ~ 0x3fffffff
	...
	for( i=0x80000000; i<0x80000004; i=i+4) {
		...
		*(unsigned int *)(i) = 0xaaaaaaaa;
		if( *(unsigned int *)(i) == 0xaaaaaaaa )	;				// pass
		else 						test_result += 1;		// fail

		*(unsigned int *)(i) = 0x55555555;
		if( *(unsigned int *)(i) == 0x55555555 )	;				// pass
		else 						test_result += 1;		// fail

		*(unsigned int *)(i) = 0x00000000;
		if( *(unsigned int *)(i) == 0x00000000 )	;				// pass
		else 						test_result += 1;		// fail
...
void dram_rw_test(int mode)
{
	...
	for( i=start; i<end; i=i+step) {
		for( j = 0 ; j < 16 ; j++){
			temp[j] = *(volatile unsigned int *)(i+4*j);
		...
		for( j = 0 ; j < 16 ; j++){
			...
			*(volatile unsigned int *)(i+4*j) = temp[j];
		}
	}
	dprintf(INFO, "\n");
...
void dram_overclock_test(int mode)
{
	...
	if(mode == 0) end = 21;		//600~800MHz
	else if(mode == 1) end = 1;	//only 660MHz
	...
	while (1)
	{
		...
		for(i = 0 ; i < end ; i++)
		{
			...
			tca_ckc_setfbusctrl(FBUS_MEM, ENABLE, dram_clock * 10000);
...
#endif
	//restore FBUS_MEM	600 MHz
	tca_ckc_setfbusctrl(FBUS_MEM, ENABLE, 600 * 10000);

	//restore FBUS_CPU	1000 MHz
	//tca_ckc_setfbusctrl(FBUS_CPU, ENABLE, 10000000);	/*FBUS_CPU

...
int erase_emmc_bootpartition(SD_SLOT_T *mmc, int slot_id)
{
	...
	printf("Boot Partition has been erased successfully!\n");
	return 0;
}
...
#if defined(MMC_SAMSUNG_KLM4G1FE3B_B001) || defined(MMC_SAMSUNG_KLM8G2FE3B_B001)
/*
 * This code is implemented by ksjung.
 *
 * To change boot partition size, 
 * vendor command determined by eMMC manufacturer such as SAMSUNG, SK Hynix has to be used to do it.
 * Before to execute this command, 
 * please ask the manufacturer who assembles your eMMC how to use the vendor command.
 * The boot partition size is OTP(One Time Programmable) area, 
 * so that you should do this work carefully. 
 *
 * This code is sample code that 
 * extends boot partition of SAMSUNG eMMC(KLM4G1FE3B-B001, KLM8G2FE3B-B001) 
 * to 4MByte(maximum size of the eMMC card).
 *
 * <== Steps for Extending Boot Partition of SAMSUNG eMMC ==>
 * Step 1-1. CMD62 + 0xEFAC62EC
 * Step 1-2. CMD62 + 0x00CBAEA7
 * Step 2.   CMD62 + BOOT_PARTITION_SIZE(Maximum value is 0x00000020)
 * Step 3.   CMD62 + RPMB_PARTITION_SIZE(This value must be identical with BOOT_PARTITION_SIZE)
 *
 * - Boot & RPMB partition re-size should be configured at the same time
 * - Two boot partition size are equal
 * - Vendor CMD changes BOOT_SIZE_MULTI/RPMB_SIZE_MULTI, max re-size :
 *   16MB(FN2x), 4MB(FN2y/FN1x), (2MB:16, 4MB:32)
 * - After re-size setting, F/W will re-format the device on next power-on time, 
 *   therefore, all user data will be cleared
 * - During the resize-sequence, no sudden power off is allowed
 * - Therefore, this process should be executed under factory process
 * - FN2x(eMMC product based on 27nm NAND Flash)
 * - FN2y(eMMC product based on 21nm NAND Flash)
 * - FN1x(eMMC product based on 19nm NAND Flash)
 *
 */
#define EXTEND_BOOT_PART_STEP_COUNT 4
int do_mmc_extend_boot_partition(PSD_SLOT_T pSlot, int cmd, int arg)
{
  int result = 0;
  unsigned int status = 0;

  result = SD_SLOT_SendCommand(pSlot, RspType1b, cmd, arg);
  if (result) {
    printf("[%s] Fail to send cmd:%d, arg:0x%08x...\n", __func__, cmd, arg);
    return -1;
  }
  ...
  if (result != 0 || check_response(pSlot)) {
    printf("[%s] Unexpected error is detected staus : 0x%08x, result : %d...\n", 
	__func__, pSlot->ulResponse[0], result);
    result = -1;
    return result;
  }

  printf("[%s] Success to send cmd(%d) with arg(0x%08x)...\n", __func__, cmd, arg);

  return 0;
}

int mmc_extend_boot_partition(void)
{
  int iSlotIndex = 0;
  int result = 0;
  int i = 0;
  ...
  if( SD_BUS_SlotIndex_Validate(iSlotIndex) == 0 ) {
    printf("[%s] SD_BUS_SlotIndex_Validate error.\n", __func__);
    return -1;
  }

  pSlot = &sSD_BUS_Slot[iSlotIndex];

  for (i=0; i<EXTEND_BOOT_PART_STEP_COUNT; i++) 
  {
    result = do_mmc_extend_boot_partition(pSlot, cmd, arg[i]);
    if (result) {
      printf("[%s] Fail to send cmd(%d) with arg(0x%08x) at loop count no.%d...\n",
	  __func__, cmd, arg[i], i);
      return result;
    }
  }
  printf("[%s] Success to extend boot partition!!\n", __func__);

  return 0;
}

int mmc_extend_boot_part_main(void)
{
  char str = 'X';

  printf("Do you want to do extend boot partition of eMMC?\n");
  printf("Please answer about this qeustion as \"Y\" or \"N\"\n");

  while (getc(&str)) { }

  if (str == 'Y' || str == 'y') return mmc_extend_boot_partition();
  else if (str == 'N' || str == 'n') printf("Extending boot partition is canceled...\n");
  else printf("main() function is unexpectedly finished...\n");
  printf("%c\n", str);

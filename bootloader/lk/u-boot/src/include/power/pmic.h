...
struct power_control {
	int (*set_power) (struct pmic *p, unsigned int id, unsigned int onoff);
	int (*set_voltage) (struct pmic *p, unsigned int id, unsigned int mV);
};
#endif
...
struct pmic {
	...
	struct power_fg *fg;
#ifdef CONFIG_TCC
	const struct power_control *ctrl;
#endif

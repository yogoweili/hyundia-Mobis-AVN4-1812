...
#include <asm/arch/fwdn/TC_File.h>
#include <asm/arch/fwdn/fwdn_drv_v7.h>
#include <asm/arch/nand_drv_v8.h>
...
#define FWDN_TRACE(fmt...)				dprintf(fmt)

extern void target_reinit(int);
extern int fwdn_mmc_update_bootloader(unsigned int rom_size, int boot_partition);
extern int FwdnSetBootSDSerial(unsigned char *ucData, unsigned int overwrite);
extern int fwdn_mmc_get_serial_num(void);
extern int erase_emmc(unsigned long start_addr, unsigned long erase_size, int low_format);
...
int FWDN_DRV_OTP_Write(unsigned char* oBuff)
{
	...
	if((otpbox.uid_flags == 1 ) && (otpbox.uid_size != 0)){
		...
		tzow_api(600, wOTPBuf, otpbox.uid_size);

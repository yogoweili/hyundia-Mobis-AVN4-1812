...
#include <asm/arch/sdmmc/sd_slot.h>
...
void SD_SLOT_InitRecord(PSD_SLOT_T pSlot)
{
	...
	pSlot->pHwSdSlotReg->usAutoCMD12ErrorStatus = 0x00000000;
...
void SD_SLOT_SetInterface(PSD_SLOT_T pSlot)
{
	...
	if( !SD_HW_IS_HIGH_SPEED( SD_BUS_GetBootSlotIndex() ) )
		SD_SLOT_SetClockRate(pSlot);
	else {
		if (SD_HW_IS_DDR_MODE( SD_BUS_GetBootSlotIndex() )) {
			printf("[mmc] Start ddr signal in host controller.\n");
			pSdSlotReg->usAutoCMD12ErrorStatus = 0x00040000;
			SD_HW_SET_DDR_CAP( SD_BUS_GetBootSlotIndex() );
			//printf("%p\n", &pSdSlotReg->usAutoCMD12ErrorStatus);
			//*(volatile unsigned long*)0x7602063c = 0x0004;
			//*(volatile unsigned long*)0x76020860 = 0x8CFF9870;
			//*(volatile unsigned long*)0x76020864 = 0x00000007;
		} else
			pSlot->ulClockRateInKHz = 48000;
		pSlot->usSdClock = (unsigned short) INTERNAL_CLOCK_ENABLE;
#if defined(CONFIG_TCC893X)
		//BITSET(pSdSlotReg->ucHostControl, (1<<2) );
#endif
	}
	SD_SLOT_ClockOff(pSlot);
	SDMMC_ms_delay(10);

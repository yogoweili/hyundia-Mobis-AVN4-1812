		    ...
		    int low_format)
{
	...
	TRACE("pSlot->ulResponse[0] : %X\n", (unsigned int)pSlot->ulResponse[0]);
	...
	do{
		...
		TRACE("CARD_STATUS : %08X\n", (unsigned int)EMMC_BOOT_CARD_STATUS(pSlot->ulResponse[0]));
...
static unsigned long get_emmc_density(PSD_SLOT_T pSlot, unsigned long erase_unit)
{
	...
	TRACE(" 2 ^ C_SIZE_MULT : %ld\n",  (long int)(2 << ((unsigned long)pSlot->csd_441.C_SIZE_MULT + 2 - 1)));

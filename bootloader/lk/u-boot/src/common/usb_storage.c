...
unsigned long usb_stor_read(int device, lbaint_t blknr,
			    lbaint_t blkcnt, void *buffer)
{
	lbaint_t start, blks, blks2;
	uintptr_t buf_addr;
	unsigned short smallblks;
	struct usb_device *dev;
	struct us_data *ss;
	int retry, i, prt_count = 0;
	ccb *srb = &usb_ccb;
	...
	blks2 = blks;
	do {
		//if(prt_count&&!(prt_count%800))	printf("\n");
		//if(prt_count &&!(prt_count%10))	printf(".");
		//if(prt_count &&!(prt_count%30))	printf("\r%lu/%lu blocks",(blks2 - blks), blks2);
		prt_count++;
...
retry_it:
	...
	//if(prt_count > 1) printf("\r%lu/%lu blocks\n",blks2, blks2);

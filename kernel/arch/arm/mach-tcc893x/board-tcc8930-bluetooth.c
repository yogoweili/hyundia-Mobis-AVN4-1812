...
**/
		gpio_set_value(TCC_GPF(15), 1);   /* BT_WAKE Enable , pwr_ctrl*/
		gpio_set_value(TCC_GPC(27), 0);   /* reset */
		msleep(500);
		gpio_set_value(TCC_GPC(27), 1);   /* reset */
...
* Bluetooth GPIO Setting
**/
		gpio_set_value(TCC_GPF(15), 0);   /* BT_WAKE Enable , pwr_ctrl*/
		gpio_set_value(TCC_GPC(27), 0);   /* reset */
	}
	else if (machine_is_m801_88() || machine_is_m803())
	{
...
static int tcc_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	int bt_ver = daudio_bt_version();
	int bt_reg_gpio = get_gpio_number(CTL_BT_REG_ON);
	int bt_reset_gpio = get_gpio_number(CTL_BT_RESET);
...
static int tcc_bluetooth_probe(struct platform_device *pdev)
{
	 ...
	 * Bluetooth GPIO Setting
	 * @date: 2014/11/4
	 * valky - BT gpio setting
	 **/
	#if defined(CONFIG_DAUDIO)
		if (bt_reg_gpio != -1) gpio_request(bt_reg_gpio, "bt_reg");
		if (bt_reset_gpio != -1) gpio_request(bt_reset_gpio, "bt_reset");

...
static unsigned int io_rate, hsio_rate;
static int max_io_enable(struct clk *clk)
{
	...
	if (io_rate)
		tca_ckc_setfbusctrl(FBUS_IO  , NOCHANGE, io_rate+1);
	else
		tca_ckc_setfbusctrl(FBUS_IO  , NOCHANGE, 1800000);
	if (hsio_rate)
		tca_ckc_setfbusctrl(FBUS_HSIO  , NOCHANGE, hsio_rate+1);
	else
...
static struct clk clk_gpu = {
    .name           = "gpu",
#if defined(CONFIG_IOBUS_DFS) && defined(CONFIG_PM_RUNTIME)
    .parent         = &clk_max_io,
#endif
    .flags          = CLK_AUTO_OFF,
    .enable         = fclk_enable,
...
static struct clk clk_overlay = {
    ...
    .enable         = pclk_enable,
    .disable        = pclk_disable,
    .pwdn_idx       = RB_OVERLAYMAXER,
    .pwdn           = tca_ckc_setiopwdn,
    .swreset_idx    = RB_OVERLAYMAXER,
...
int clk_check(struct clk *clk)
{
	return clk->usecount;
}
EXPORT_SYMBOL(clk_check);
...
#if defined(CONFIG_IOBUS_DFS)
	io_rate = tca_ckc_getfbusctrl(FBUS_IO);
	hsio_rate = tca_ckc_getfbusctrl(FBUS_HSIO);
#endif
    mutex_unlock(&clock_mutex);

...
#include <asm/cpu_pm.h>         // cpu_pm_enter(), cpu_pm_exit()
...
// GPIO Restore
	...
	for(cnt=0; cnt<(sizeof(GPIO)/sizeof(unsigned)); cnt++) {
		dest[cnt] = src[cnt];
		#if defined(CONFIG_TCC_CM3_USE)
			if (cnt == 4)    cnt++; //20130821 ysseung   to not restore GPIO_F.
		#endif
	}
...
// PLL25 Power OFF
	...
	else if(*(volatile unsigned long *)SRAM_STACK_ADDR == 0x5001 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x5002){
...
//Bruce, wake-up source should be surely set here !!!!
	...
	if(*(volatile unsigned long *)SRAM_STACK_ADDR == 0x1000)
	{
		...
		if(*(volatile unsigned long *)(SRAM_STACK_ADDR+4) == 1)		// SD Insert -> Remove in suspend : Active High
			((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_B13 = 1;	// PMU WakeUp Enable
		#endif
	}
	else if(*(volatile unsigned long *)SRAM_STACK_ADDR == 0x2000 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x3000)
	{
	...
	else if(*(volatile unsigned long *)SRAM_STACK_ADDR == 0x5000 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x5001 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x5002)
	{
		//set wake-up polarity
...
static void tcc_pm_power_off(void)
{
	...
	axp202_power_off();
#elif defined(CONFIG_REGULATOR_RT5027)
...
static void tcc_pm_restart(char str)
{
	...
	printk(KERN_ERR "[KDEBUG] %s reboot: reason=0x%x\n",__func__, restart_reason);
...
static int tcc_reboot_call(struct notifier_block *this, unsigned long code, void *cmd)
{
	...
	printk(KERN_ERR "[KDEBUG] %s code=%d, cmd = %s \n",__func__, code,cmd);
	...
	if (code == SYS_RESTART) {
		if (cmd) {
			...
			} else if (!strcmp(cmd, "force_normal")) { /* enter normalboot mode forcely */
				restart_reason = 3;
			} else {
				restart_reason = 0;
			}
		...
		if( (restart_reason >= 0) && (restart_reason <= 3) ) {
			...
			strlcpy(write_Msg.command, spReason[restart_reason], sizeof(write_Msg.command));
			...
			for (loop = 0; loop < 100; loop++) {
				...
				if (fd >= 0) {
					...
					if (result >= 0) {
						...
						if (strncmp(spReason[restart_reason],read_Msg.command,strlen(spReason[restart_reason])) == 0) {
...
{
	//bluetooth power off
	gpio_set_value(CTL_BT_RESET, 0);
	//sxm shutdown
	gpio_set_value(SIRIUS_GPIO_RESET, 1);
	mdelay(5);
#if defined(INCLUDE_XM)
	tcc_gpio_config(TCC_GPD(13), GPIO_FN(0)|GPIO_OUTPUT|GPIO_LOW|GPIO_PULL_DISABLE);
	tcc_gpio_config(TCC_GPD(14), GPIO_FN(0)|GPIO_OUTPUT|GPIO_LOW|GPIO_PULL_DISABLE);
#endif

...
//    tx			   rx			     rts			cts			  fn
	...
	{TCC_GPB(25), TCC_GPB(26),                  0,                   0, GPIO_FN(10)}, // 12  // UT_TXD(8)
...
//    tx			   rx			     rts			cts			  fn
	...
	{0, 				0,                  0,                  0, 0}, 					// 9   
	{TCC_GPC(10), TCC_GPC(11),                  0,                  0, GPIO_FN(6)}, // 10  // UT_TXD(10)	
...
#elif defined(CONFIG_MACH_M805_893X) || defined(CONFIG_MACH_TCC8930ST)
		//control LDO
		if (on) {
			gpio_direction_output (sc_io.pwren, 1);
			gpio_set_value (sc_io.pwren, 1);
...
void tca_sc_init_port(void)
{
...
#if defined(CONFIG_CHIP_TCC8935) || defined(CONFIG_CHIP_TCC8933) || defined(CONFIG_CHIP_TCC8933S) || defined(CONFIG_CHIP_TCC8937S)
	...
	sc_io.rst = TCC_GPEXT2(6);		//active-low
	sc_io.detect = TCC_GPE(18);		//active-high
	sc_io.pwren = TCC_GPEXT2(7);		//active-low
	sc_io.set_volt = -1;
	sc_io.clk = TCC_GPC(11);
	sc_io.data = TCC_GPC(10);
 ...
 **/
	...
	sc_io.rst       = TCC_GPA(4);		//active-low
	sc_io.detect    = TCC_GPB(16);		//active-high
	sc_io.pwren     = TCC_GPA(4);		//active-low
#else
	sc_io.rst = TCC_GPEXT2(6);		//active-low
	sc_io.detect = TCC_GPB(16);		//active-high
	sc_io.pwren = TCC_GPEXT2(7);		//active-low
#endif
	sc_io.set_volt = -1;
	sc_io.clk = TCC_GPC(23);
	sc_io.data = TCC_GPC(22);
	...
	if (sc_io.rst != -1) {
		tcc_gpio_config(sc_io.rst, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
		gpio_direction_input (sc_io.rst);
	}
	if (sc_io.detect != -1) tcc_gpio_config(sc_io.detect, GPIO_FN(0)|GPIO_INPUT); 
	if (sc_io.clk != -1) tcc_gpio_config(sc_io.clk, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
	if (sc_io.data != -1) tcc_gpio_config(sc_io.data, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
...
#elif defined (CONFIG_MACH_TCC8930ST)
	uUartPort = 8;
#else defined (CONFIG_MACH_M805_893X)
	uUartPort = 11;
#endif
...
int tca_sc_disable(void)
{
	...
	if (sc_io.data != -1) tcc_gpio_config(sc_io.data, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
	if (sc_io.clk != -1) tcc_gpio_config(sc_io.clk, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
	if (sc_io.rst != -1) tcc_gpio_config(sc_io.clk, GPIO_FN(0)|GPIO_INPUT|GPIO_PULLUP);
...
#if defined(CONFIG_MACH_M805_893X)  || defined(CONFIG_MACH_TCC8930ST)

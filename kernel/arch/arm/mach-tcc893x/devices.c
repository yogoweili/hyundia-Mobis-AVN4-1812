...
static struct resource uart6_resources[] = {
    [0] = {
        .start  = tcc_p2v(HwUART6_BASE),
        .end    = tcc_p2v(HwUART6_BASE) + 0xff,
        .flags  = IORESOURCE_MEM,
    },
    [1] = {
        .start  = INT_UART6,
...
struct platform_device tcc8930_uart6_device = {
    .name           = "tcc-uart",
    .id             = 6,
...
static int tcc_usb30_phy_init(struct platform_device *pdev, int type)
{
	...
	//	BITCSET(USBPHYCFG->UPCR2, TXVRT_MASK, 0xC << TXVRT_SHIFT);
		USBPHYCFG->UPCR2 = 0xF19F9F00;	// 2014/08/26, USB device eyepattern tuning by H/W team.
      ...
      uTmp = readl(tcc_p2v(0x7100C000+0x164));
		uTmp = (uTmp>>18)&0xF;
...
static struct resource spi1_resources[] = {
	...
	[2] = {	    
         ...
         * @ author sjpark@cleinsoft
         * @ date 2013/11/19
         * spi setting for daudio - gpio port 2 (XM)
         * @ date 2015/1/16
         * XM - port 2 : GPIO_D[22:25]
         * CMMB - port 2 : GPIO_E[17:20]
         **/
#if defined(INCLUDE_XM)
        .start  = 2, /* Port2 GPIO_D[22:25] */
        .end    = 2,
#elif defined(INCLUDE_CMMB)
        .start  = 12, /* Port12 GPIO_E[17:20] */
        .end    = 12,
#else
        .start  = 2,
        .end    = 2,
...
// tsif2 : tsif slave : for TPEG
static struct resource tsif2_resources[] = {
	[0] = {
...
struct platform_device tcc_tsif2_device = {
	.name		= "tcc-tsif",
	.id		= 2,
	.resource	= tsif2_resources,
	.num_resources	= ARRAY_SIZE(tsif2_resources),
};

// tsif1 : tsif slave : for TDMB
...
struct platform_device tcc_tsif1_device = {
   .name       = "tcc-tsif",
   .id     = 1,
...
// tsif0 : tsif slave : only for MICOM
...
struct platform_device tcc_tsif0_device = {
	.name		= "tcc-tsif",
	.id		= 0,
...
* @ sjpark@cleinsoft
* @ date 2013.08.23 
* @ add platform device for fm1288 codec
**/
#if defined(CONFIG_SND_SOC_NXP)
static struct platform_device tcc_nxp = {
	        .name   = "tcc-nxp",
			.id = -1,
};
#endif
#if defined(CONFIG_SND_SOC_FM1288)
static struct platform_device tcc_fm1288 = {
	    .name   = "tcc-fm1288",
		    .id = -1,
};
#endif
...
* @ date 2013.08.23 
* @ add platform device for fm1288 codec
* @ add platform device for nxp codec
**/
#if defined(CONFIG_SND_SOC_FM1288)
	platform_device_register(&tcc_fm1288);
#endif
#if defined(CONFIG_SND_SOC_NXP)
	platform_device_register(&tcc_nxp);
#endif
}
#else
static void tcc_init_audio(void){;}
...
#if defined(CONFIG_SIRIUS_CONTROL)
static struct platform_device daudio_sirius = {
	.name	= "sirius_dev",
	.id		= -1,
};
#endif

#if defined(CONFIG_DAUDIO_PIN_CONTROL)
static struct platform_device daudio_pin_ctl = {
	.name	= "daudio_pinctl",
	.id		= -1,
};
#endif

#if defined(CONFIG_TCC_CMMB_SUPPORT)
static struct platform_device tcc_cmmb_ctl = {
	.name	= "tcc_cmmb",
	.id		= -1,
};
#endif
...
#if defined(CONFIG_SIRIUS_CONTROL)
	platform_device_register(&daudio_sirius);
#endif

#if defined(CONFIG_DAUDIO_PIN_CONTROL)
	platform_device_register(&daudio_pin_ctl);
#endif

#if defined(CONFIG_TCC_CMMB_SUPPORT)
	platform_device_register(&tcc_cmmb_ctl);
#endif

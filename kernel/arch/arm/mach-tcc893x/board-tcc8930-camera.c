...
static int get_cam_slave_id(void)
{
	int id = 0;
	...
	switch (ver)
	{
		...
		default:
			...
			id = (TW8836_SENSOR_I2C_SLAVE_ID >> 1);
#else
			id = (TW9912_SENSOR_I2C_SLAVE_ID >> 1);
#endif
			break;
	}
#else
	id = SENSOR_I2C_SLAVE_ID;
#endif
...
static struct i2c_board_info __initdata i2c_camera_devices[] = {
	{
		I2C_BOARD_INFO("tcc-cam-sensor", 0),
...
void __init tcc8930_init_camera(void)
{
	...
	volatile GPION *gpioe = (volatile GPION*)tcc_p2v(HwGPIOE_BASE);

	if (!machine_is_tcc893x())
		return;

	BITSET(gpiod->GPIS.nREG, 0x1FF);	//Schimitt Input Function enable on GPD0 ~ GPD8
	BITSET(gpioe->GPIS.nREG, 0x4FF);	//Schimitt Input Function enable on GPE0 ~ GPE7, GPE10

	i2c_camera_devices[0].addr = get_cam_slave_id();
	i2c_register_board_info(3, i2c_camera_devices, ARRAY_SIZE(i2c_camera_devices));
}

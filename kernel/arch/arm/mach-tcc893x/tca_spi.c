...
#define TCC893X_SPI_BIT_WIDTH 32
#define SPI_BIT_WIDTH TCC893X_SPI_BIT_WIDTH
...
static void tca_spi_setpacketcnt_slave(struct tca_spi_handle *h, int size)
{
	...
	//printk("packet count - dma total packet cnt : %d\n", packet_cnt);
...
* @date 2013/12/20
* spi setting for daudio - CFGRTH register cleared. 
* 000 -> CFGRTH + 1 -> request dma interrupt when FIFO received 1*32bits 
* ( FIFO size can be configured 4*32bits or 8*16bits : current setting - 4*32bits)
* 001 -> CFGRTH + 1 -> request dma interrupt when FIFO received 2*32bits 
**/
...
* @author sjpark@cleinsoft
* @date 2014/12/15
* ioctl sets the bit width of spi comm. 
**/
#else
    h->set_bit_width(h, SPI_BIT_WIDTH);
#endif
    h->set_dma_addr(h);
...
* @author sjpark@cleinsoft
* @date 2014/12/15
* spi setting for daudio - set gpio port 2 (DMB)
**/
...
* @author sjpark@cleinsoft
* @date 2014/12/15
* spi setting for daudio - set gpio port 5 (MICOM)
**/
...
* @author sjpark@cleinsoft
* @date 2014/12/15
* spi setting for daudio - set gpio port 5 (TPEG)
**/

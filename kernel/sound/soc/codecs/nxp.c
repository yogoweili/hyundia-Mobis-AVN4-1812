...
static __devinit int nxp_codec_probe(struct platform_device *pdev)
{
	int ret;
	print_func;
...
static int __devexit nxp_codec_remove(struct platform_device *pdev)
{
...
static struct platform_driver nxp_codec_driver = {
	...
	.remove = __devexit_p(nxp_codec_remove),
};

...
static int __init tcc_init_wm8524(void)
{
    ...
    alsa_dbg("TCC Board probe [%s]\n", __FUNCTION__);
		...
		if(machine_is_tcc8920st() || machine_is_tcc8930st()) {
		        ...
		        gpio_direction_output(TCC_GPB(7), 1);   /* MUT_ANG: h/w mute */
		        gpio_direction_output(TCC_GPB(6), 0);   /* MUTE_CTL of WM8524 */
			#elif defined(CONFIG_STB_BOARD_YJ8925T) || defined(CONFIG_STB_BOARD_YJ8935T) || defined(CONFIG_STB_BOARD_YJ8933T)
		        gpio_direction_output(TCC_GPE(26), 1);  /* UT_ANG: h/w mute */
			#elif defined(CONFIG_STB_BOARD_YJ8930T)
		        gpio_direction_output(TCC_GPB(6), 1);   /* UT_ANG: h/w mute */
			#endif

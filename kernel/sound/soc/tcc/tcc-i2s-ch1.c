...
* @date 2014/1/20
* New Sound System - Audio 1 (Master mode)
**/
    /* set DAI register */
#if defined(CONFIG_DAUDIO) && defined(CONFIG_SND_SOC_DAUDIO_NEW_SOUND_SYSTEM)
...
#ifdef TCC_MULTIOUTPUT_SUPPORT
	if ((substream->pcm->device == __I2S_DEV_NUM__) || (substream->pcm->device == __I2S_SECOND_DEV_NUM__)) {
#else
...
#ifdef TCC_MULTIOUTPUT_SUPPORT
	if ((substream->pcm->device == __SPDIF_DEV_NUM__)|| (substream->pcm->device == __SPDIF_SECOND_DEV_NUM__)) {
#else
...
{
    int ret = 0;
	volatile PADMADAI1 pDAI  = (volatile PADMADAI1)tcc_p2v(BASE_ADDR_DAI1);
    alsa_dbg(" %s \n", __func__);
...
static int tcc_i2s_suspend(struct snd_soc_dai *dai)
{
    ...
    alsa_dbg(" %s \n", __func__);
#ifdef TCC_MULTIOUTPUT_SUPPORT
	if(dai->id == 2) {  // DAI  // QB Issue fix Planet_20131021
#else
    if(dai->id == 0) {  // DAI
#endif
...
* @author sjpark@cleinsoft
* @date 2014/03/06
* CDIF Audio Format Set - CDIF Audio Channel changed from audio 0 to audio 1 
**/
#if defined(CONFIG_DAUDIO) && defined(CONFIG_SND_SOC_DAUDIO_NEW_SOUND_SYSTEM)
			.formats = SNDRV_PCM_FMTBIT_U16_LE| SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_IEC958_SUBFRAME_LE,},  //should be change? phys:32 width:16
#else

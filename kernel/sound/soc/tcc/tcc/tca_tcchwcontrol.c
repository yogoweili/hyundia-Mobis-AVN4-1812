...
{
    volatile GPION *gpiog = (volatile GPION*)tcc_p2v(HwGPIOG_BASE); 
...
* 	@date 2013/08/23
* 	porting fm1288 audio codec
*   New Sound System - gpio function updated (2014/1/20)
*   New Sound System for daudio 3rd BD - gpio function updated (2014/2/24)
**/
#if defined(CONFIG_DAUDIO)
#if defined(CONFIG_SND_SOC_DAUDIO_NEW_SOUND_SYSTEM)
			// ==================  AUDIO CHANNEL 0 =================
			// AUDIO 0 CLOCK
			tcc_gpio_config(TCC_GPG(7), GPIO_FN1 ); // MBCLK
			tcc_gpio_config(TCC_GPG(8), GPIO_FN1 ); // MLRCK
			// AUDIO 0 DOUT - HF OUTPUT
			tcc_gpio_config(TCC_GPG(9), GPIO_FN1); // MDOUT 0(HF OUT)
			// AUDIO 0 DIN - HF INPUT (MIC)
			tcc_gpio_config(TCC_GPG(10), GPIO_FN1 ); // MDIN 0(MIC)
			// AUDIO 0 DIN - BT OUTPUT
			tcc_gpio_config(TCC_GPG(11), GPIO_FN1); // MDOUT 1(BT)
			// AUDIO 0 DIN - BT INPUT
			tcc_gpio_config(TCC_GPG(12), GPIO_FN1 ); // MDIN 1(BT)
			// AUDIO 0 DIN - MODEM OUTPUT
			tcc_gpio_config(TCC_GPG(13), GPIO_FN1); // MDOUT 2(MODEM)
			// AUDIO 0 DIN - MODEM INPUT
			tcc_gpio_config(TCC_GPG(14), GPIO_FN1 ); // MDIN 2(MODEM)

/**
*	@author sjpark@cleinsoft
* 	@date 2014/11/21
* 	Setting the spdif rx gpio properties to reduce rx signal problem. (Sound Hound)
* 	Drive strength & Schmitt Input & Slew Rate  
**/
            // AUDIO 0 MSPDIF_RX - SOUND HOUND / DIPO
            tcc_gpio_config(TCC_GPG(17), GPIO_FN1 | GPIO_CD(3)); //MSPDIF_RX(SOUND HOUND/DIPO)
            gpiog->GPSR.nREG |= Hw17|Hw14|Hw12|Hw10|Hw8|Hw7; //Slew Rate
            gpiog->GPIS.nREG |= Hw17|Hw14|Hw12|Hw10|Hw8|Hw7; //Schmitt Input
	
            //printk("gpiog->GPSR.nREG : 0X%X\n", gpiog->GPSR.nREG); 
            //printk("gpiog->GPIR.nREG : 0X%X\n", gpiog->GPIS.nREG);

            tcc_gpio_config(TCC_GPG(18), GPIO_FN1); //MSPDIF_TX(NAVI)
			// ==================  AUDIO CHANNEL 1 =================
			// AUDIO 1 CLOCK
			tcc_gpio_config(TCC_GPG(1), GPIO_FN1); //BCLK
			tcc_gpio_config(TCC_GPG(2), GPIO_FN1); //LRCK
			// AUDIO 1 DOUT - MM & CD
			tcc_gpio_config(TCC_GPG(3), GPIO_FN1); // DOUT 0(MM & CD)
			// CDIF
			tcc_gpio_config(TCC_GPC(27), GPIO_FN1); // CD_BCLKI
			tcc_gpio_config(TCC_GPC(28), GPIO_FN1); // CD_LRCKI
			tcc_gpio_config(TCC_GPC(9), GPIO_FN1); // CD_DI
#else
			// AUDIO0 I2S(MEDIA)
			tcc_gpio_config(TCC_GPG(7), GPIO_FN1); // MBCLK
			tcc_gpio_config(TCC_GPG(8), GPIO_FN1); // MLRCK
			tcc_gpio_config(TCC_GPG(9), GPIO_FN1); // MDOUT
			// AUDIO0 MSPDIF_RX
			tcc_gpio_config(TCC_GPG(17), GPIO_FN1); //MSPDIF_RX(SOUND HOUND)
			//AUDIO1 I2S (MIC)
			tcc_gpio_config(TCC_GPG(1), GPIO_FN1); //BCLK
			tcc_gpio_config(TCC_GPG(2), GPIO_FN1); //LRCK
			tcc_gpio_config(TCC_GPG(3), GPIO_FN1); //DAO
			tcc_gpio_config(TCC_GPG(4), GPIO_FN1); //DAI
			//AUDIO1 SPDIF TX
			tcc_gpio_config(TCC_GPG(5), GPIO_FN1); //SPDIF_TX (NAVI)
			// CDP
			tcc_gpio_config(TCC_GPE(29), GPIO_FN3); // MCD_BCLKI
			tcc_gpio_config(TCC_GPE(30), GPIO_FN3); // MCD_LRCKI
			tcc_gpio_config(TCC_GPE(31), GPIO_FN3); // MCD_DI
#endif
		}
#else
...
*	@author sjpark@cleinsoft
* 	@date 2014/07/10
*   Clear DAI master enable register to control audio clock 
**/
...
//		| Hw4	//DAI LRCLK(32fs->fs)
//		| Hw5	//DAI LRCLK(32fs->fs)
//		| Hw6   //DAI Bit Clock Divider Select (256fs->32fs)	
		| Hw7   //DAI Bit Clock Divider Select (256fs->32fs)
		;
    return(0);
}

unsigned int tca_i2s_dai_init_slave_mode(void *pADMADAIBaseAddr)
{
	ADMADAI *pStrADMADAIBaseReg = (ADMADAI *)pADMADAIBaseAddr;
...
*	@author sjpark@cleinsoft
* 	@date 2013/10/23
*   set dai register setting for slave mode
**/
	pStrADMADAIBaseReg->DAMR = 0
//		| Hw31 	//BCLK direct at master mode
//		| Hw30 	//LRCLK direct at master mode	

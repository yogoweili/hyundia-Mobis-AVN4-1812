...
//    dma_buffer = dma_buffer * (((length) >> 4) << 8);
    ...
    //pADMA->RptCtrl = 0;	// TCC_MULTIOUTPUT_SUPPORT 20130711
...
**/
#if defined(CONFIG_DAUDIO) && defined(CONFIG_SND_SOC_DAUDIO_NEW_SOUND_SYSTEM)
	volatile PADMACDIF pADMACDIF = (volatile PADMACDIF)tcc_p2v(BASE_ADDR_CDIF1); 
#endif
...
#ifdef TCC_MULTIOUTPUT_SUPPORT
	if(substream->pcm->device < __I2S_SECOND_DEV_NUM__) {
		...
		alsa_dbg("[%s] close %s device, %s\n", __func__, substream->pcm->device == __SPDIF_SECOND_DEV_NUM__ ? "spdif":"pcm", 
													substream->stream == SNDRV_PCM_STREAM_PLAYBACK ? "output" : "input");
	}
#else

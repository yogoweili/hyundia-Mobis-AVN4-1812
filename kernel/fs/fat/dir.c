 ...
 ** @date 2014/10/20      
 ** readdir2 function porting - __fat_readdir_avn 
 ** This function be called if user calls read_dir2 on the FAT filesystem
 **/  
static int __fat_readdir_avn(struct inode *inode, struct file *filp, void *dirent,
			 filldir_avn_t filldir, int short_only, int both)
{
	struct super_block *sb = inode->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	struct buffer_head *bh;
	...
	if (inode->i_ino == MSDOS_ROOT_INO) {
		while (cpos < 2) {
			if (filldir(dirent, "..", cpos+1, cpos, MSDOS_ROOT_INO, DT_DIR, 0, 0, 0) < 0)
				goto out;
			cpos++;
			filp->f_pos++;
		}
...
start_filldir:
	...
	if (!memcmp(de->name, MSDOS_DOT, MSDOS_NAME)) {
		...
		loff_t i_pos = fat_make_i_pos(sb, bh, de);
		struct inode *tmp = fat_build_inode(sb, de, i_pos);
		if (tmp) {
			inum = tmp->i_ino;
			mode = tmp->i_mode;
			mtime = tmp->i_mtime.tv_sec;
			filesize = tmp->i_size;
			iput(tmp);			
		} else {
			inum = iunique(sb, MSDOS_ROOT_INO);
	...
	if (filldir(dirent, fill_name, fill_len, *furrfu, inum,
		    (de->attr & ATTR_DIR) ? DT_DIR : DT_REG, mode, mtime, filesize) < 0)
		goto fill_failed;
 ...
 ** @author sjpark@cleinsoft
 ** @date 2014/10/20      
 ** readdir2 function porting - fat_readdir_avn 
 **/  
static int fat_readdir_avn(struct file *filp, void *dirent, filldir_avn_t filldir)
{
	struct inode *inode = filp->f_path.dentry->d_inode;
	return __fat_readdir_avn(inode, filp, dirent, filldir, 0, 0);
}
 ...
 ** @author sjpark@cleinsoft
 ** @date 2014/10/20      
 ** readdir2 function porting - register ops function 
 **/  
	.readdir_avn	= fat_readdir_avn,

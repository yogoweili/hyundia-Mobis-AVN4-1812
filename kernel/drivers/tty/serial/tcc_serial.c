...
CLOG_INIT(CLOG_TAG_ORGANIZATION, CLOG_TAG_UART, CLOG_LEVEL_UART);
...
void my_uart_rx_process(unsigned long arg)
{
    ...
    if (current_head != current_tail) {
        ...
        if (rts_state > (SERIAL_RX_DMA_BUF_SIZE/2)) {
            CLOG(CLL_INFO, "RTS HIGH[0x%x : 0x%x]\n", cur_addr, current_tail);
...
{
	volatile int time_out_exception = 300000;
	
	while (!(rd_regl(port, OFFSET_LSR) & LSR_THRE)){
		barrier();
		time_out_exception--;
		if(time_out_exception < 0) break;
	}				
...
static void tcc_serial_start_tx(struct uart_port *port)
{
	...
	if (tcc_port->tx_dma_use) {
        ...
        if(tcc_port->tx_done) {
	     ...
	     //tcc_serial_tx(port); //M150121_02_1051_TCC_PATCH_UART_CPU_LOAD_ERROR_FIX	
             wr_regl(port, OFFSET_IER, rd_regl(port, OFFSET_IER) | IER_ETXI);
	}
}
 ...
 **/
        ...
        //printk("%s:%s TODO: gpio port mapping", __FILE__, __FUNCTION__);
        gpio_request(TCC_GPA(4), "BT ON");
        gpio_direction_output(TCC_GPA(4), 1);
#else
    ...
    if(tcc_port->rx_dma_use) {
        ...
        if(tcc_port->modem_control_use == 1)
...
static void tcc_serial_set_baud(struct tcc_uart_port *tcc_port, unsigned int baud)
{
    ...
    switch(baud) {
	case 921600:
	    clk_set_rate(tcc_port->clk, 103219200);		// 104.219MHz
	    break;
	case 1444444:
	    clk_set_rate(tcc_port->clk, 92444416);		// 92.444MHz
            break;
	case 2500000:
	    clk_set_rate(tcc_port->clk, 120*1000*1000);		// 120MHz
            break;
        default :
	      clk_set_rate(tcc_port->clk, 96*1000*1000);	// 96MHz
            break;
    }
}
				       ...
				       struct ktermios *old)
{
	...
	CLOG(CLL_INFO, "uart %02d setting ulcon: %08x, umcon: %08x, brddiv to %d, baud %d, uart_clk %d\n", port->line, ulcon, umcon, quot, baud, uart_clk);
	spin_lock_irqsave(&port->lock, flags);
...
static int tcc_serial_probe(struct platform_device *dev)
{
    ...
    if(tcc_platform_data->rx_dma_use)
    {
        ...
        tcc_port->modem_control_use = tcc_platform_data->modem_control_use;

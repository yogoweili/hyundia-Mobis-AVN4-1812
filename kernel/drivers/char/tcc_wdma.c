...
char tccxxx_wdma_ctrl(unsigned int argp)
{
	...
	ImageCfg.TargetWidth = panel->xres;
	ImageCfg.TargetHeight = panel->yres;
	// ImageCfg.ImgFormat = TCC_LCDC_IMG_FMT_UYVY; be filled by caller
	
	// change baseaddres to pmap of overlay1
	base_addr = (unsigned int )ImageCfg.BaseAddress;	// BaseAddress is filled by caller
...
int tccxxx_wdma_release(struct inode *inode, struct file *filp)
{
	...
	//VIOC_CONFIG_PlugOut(VIOC_SC0); // 20150420 JTS

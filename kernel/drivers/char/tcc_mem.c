...
long tmem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TCC_8925S_IS_XX_CHIP:
		{
			...
			unsigned int RegAddr = 0xFB000000;//phy : 0x30000000)
	
			if( *(volatile unsigned int *)(RegAddr + 0x1C) == 0x12080600)
				ret= 1;
#endif
		}
		break;
		...
		case QB_UART_LOG:
		{
			...
			const char *log_str = (const char *)arg;
			printk(KERN_ERR "\x1b[1;31mUART - %s\x1b[0m\n", log_str);
			ret = 1;
			break;
		}
		case QB_BOOT_TIME:
		{
			...
			int *boot_time = (int *)arg;
			int *lk_time = &boot_time[0];
			int *kernel_time = &boot_time[1];
			int *frameworks_time = &boot_time[2];

			*lk_time = lk_boot_time;
			*kernel_time = kernel_boot_time;

			frameworks_boot_time = cpu_clock(0) - frameworks_boot_time;	// CPU Core 0's kick time.
			do_div(frameworks_boot_time, 1000000);
			
			*frameworks_time = (int)frameworks_boot_time;

			ret = 1;
			break;
		}
		default:
			dprintk("Unsupported cmd(0x%x) for tmem_ioctl. \n");
			ret = -EFAULT;
			break;
	}

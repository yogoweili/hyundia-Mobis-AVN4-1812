...
CLOG_INIT(CLOG_TAG_CLEINSOFT, CLOG_TAG_IE, CLOG_LEVEL_IE);
...
static int daudio_get_ie_level(int mode, char *level);
...
static int init_ie_settings(void)
{
	...
	{
		CLOG(CLL_ERROR, "%s setting read failed!\n", __func__);
	}
...
static int daudio_set_ie_level(int mode, char level)
{
	...
	if (mode >= SET_BRIGHTNESS && mode <= SET_AUX_SATURATION) 
	{
		...
		mutex_lock(&ie_lock);
		CLOG(CLL_CRITICAL, "%s, twxxxx_set_ie mode =%d , level = %d \n ", __func__,mode,level );
	...
	{
		CLOG(CLL_CRITICAL, "%s, TCC  mod =%d , level =%d   \n ", __func__ ,mode,level);
	...
	{	
		CLOG(CLL_CRITICAL, "%s, fail mode = %d , level = %d--------------------->> input error  \n", __func__, mode , level );
	}
	return ret;
}
...
{
	int ret = FAIL;
#if defined(CHECK_SETTING_VALUE)
	int *plevel = (int *)
			(&ie_info.tw8836_brightness + (mode - GET_BRIGHTNESS));
#endif
...
/**/
		...
		CLOG(CLL_CRITICAL, "%s level: %d, ret: %d\n", __func__, (int)(*level), ret);
	}
...
int daudio_set_ie(int mode, char level)
{
	int ret = 0;
	char hlevel = level;
...
int daudio_get_ie(int mode, char *level)
{
	int ret = FAIL;
...
static long daudio_ie_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char level1 = 0;
	int ret = FAIL;
	ie_setting_info *t_ie_info=(ie_setting_info *)arg;

	CLOG(CLL_CRITICAL, "%s cmd: %d arg: 0x%lx\n", __func__, cmd, arg);

	//copy_from_user(&t_ie_info, (ie_setting_info*)arg, sizeof(ie_setting_info));

	if (cmd < SET_BRIGHTNESS || cmd > GET_TCC_USB_SATURATION )
	{
	...
	if (cmd < GET_BRIGHTNESS)
	{
		...
		level = (char)(plevel[size_id + size_version + count]);

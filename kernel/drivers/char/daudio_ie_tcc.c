...
CLOG_INIT(CLOG_TAG_CLEINSOFT, CLOG_TAG_IE_TCC, CLOG_LEVEL_IE_TCC);
...
static int _tcc_brightness = 128;
static int _tcc_contrast = 128;
static int _tcc_gamma = 128;
		...
		unsigned char gamma, unsigned char saturation)
{
	...
	if (contrast == 0)		contrast = 128;
	if (gamma == 0)			gamma = 128;

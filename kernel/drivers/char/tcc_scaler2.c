...
char M2M_Scaler2_Ctrl_Detail(SCALER_TYPE *scale_img)
{
	...
	{
		VIOC_RDMA_SetImageSize(SC2_pRDMABase, (scale_img->src_winRight - scale_img->src_winLeft), (scale_img->src_winBottom - scale_img->src_winTop)/2);
		VIOC_RDMA_SetImageOffset(SC2_pRDMABase, scale_img->src_fmt, scale_img->src_ImgWidth*2);
	}
	else
	{
	...
	VIOC_WDMA_SetImageR2YEnable(SC2_pWDMABase, scale_img->wdma_r2y);
...
tccxxx_scaler2_init(void)
{
	...
	scaler2_clk = clk_get(NULL, "lcdc0");

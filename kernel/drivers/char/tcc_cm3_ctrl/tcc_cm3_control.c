...
static int cm3_exception = 0;

#if defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
#define	DAUDIO_DISABLE_BT656		0x8000002
extern void datv_close_cam(void);
#endif
...
CLOG_INIT(CLOG_TAG_ORGANIZATION, CLOG_TAG_CM3, CLOG_LEVEL_CM3);
...
//    BITSET( pMailBox->uMBOX_CTL_016.nREG, Hw5); //OEN high
	...
	#if 1
		ret = wait_event_interruptible_timeout( wq_cm3_cmd, cm3_cmd_status == 0, msecs_to_jiffies(5000)) ;
	#else
		ret = wait_event_interruptible( wq_cm3_cmd, cm3_cmd_status == 0) ;
	#endif
	...
	switch (cmd) {
		...
		case IOCTL_CM3_CTRL_ON:
		{
			if (cm3_clk == NULL) {
				...
				if (IS_ERR(cm3_clk)) {
					CLOG(CLL_ERROR, "can't find cortex-m3 clk driver!");
					clk_put(cm3_clk);
					cm3_clk = NULL;
					return -1;
				} else {
					if (clk_check(cm3_clk) == 0) {
						clk_set_rate(cm3_clk, 100*1000*1000); // 100MHz
						clk_enable(cm3_clk);
						cm3_exception = 1;
						return 0;
					}
					...
					if (ret < 0) {
						CLOG(CLL_ERROR, "error : request_irq %d\n", ret);
					}
				}
			}
		}
		...
		case IOCTL_CM3_CTRL_CMD:
		{
			if (cm3_clk && cm3_proc) {
				...
				CLOG(CLL_INFO, "cm3 not initialized\n");
			}
		}
		break;

	    case IOCTL_CM3_CTRL_CHECK_STATUS:
		{
			t_cm3_avn_cmd cm3_cmd;

			copy_from_user(&cm3_cmd, (t_cm3_avn_cmd *)arg, sizeof(t_cm3_avn_cmd));

			if (cm3_clk) {
				ret = cm3_cmd.pParam1 = clk_check(cm3_clk);
			} else {
				ret = cm3_cmd.pParam1 = 0;
			}

			copy_to_user((t_cm3_avn_cmd *)arg, &cm3_cmd, sizeof(t_cm3_avn_cmd));
		}
		break;
		...
		default:
			CLOG(CLL_ERROR, "cm3 error: unrecognized ioctl (0x%x)\n", cmd);
			return -EINVAL;
		break;
	}
...
static int tcc_cm3_probe(struct platform_device *pdev)
{
	...
	if(NULL == device_create(tcc_cm3_ctrl_class, NULL, MKDEV(CM3_CTRL_DEV_MAJOR, CM3_CTRL_DEV_MINOR), NULL, CM3_CTRL_DEV_NAME))
		CLOG(CLL_ERROR, "%s device_create failed\n", __FUNCTION__);
...
{
	platform_driver_unregister(&tcc_cm3_ctrl);
}

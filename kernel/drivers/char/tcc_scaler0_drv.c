...
char tccxxx_scaler0_run(SCALER_TYPE *scale_img)
{
	...
	if (gLUTUse) {
		...
		BITCSET(pLUT->uCOMP1CFG.nREG, 0x000000FF, VIOC_WMIX_WDMA_08 /* VIOC_LUT_WDMA_08 (28) */);
		BITCSET(pLUT->uCOMP1CFG.nREG, 0x80000000, 1 << 31);
	}
...
{  	
	...
	if (gLUTUse) {
		VIOC_LUT* pLUT =(VIOC_LUT *)tcc_p2v(HwVIOC_LUT);
		BITCSET(pLUT->uCOMP1CFG.nREG, 0x000000FF, 0x00);
		BITCSET(pLUT->uCOMP1CFG.nREG, 0x80000000, FALSE);
		gLUTUse = 0;
	}
...
static int tccxxx_scaler0_probe(struct platform_device *pdev)
{

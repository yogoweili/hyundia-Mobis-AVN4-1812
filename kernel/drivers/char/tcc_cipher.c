...
void tcc_cipher_set_opmode(unsigned uOpMode, unsigned uDmaMode)
{
	dprintk("%s\n", __func__);
	tca_cipher_set_opmode(uOpMode, uDmaMode);
}
		...
		case TCC_CIPHER_IOCTL_SET_ALGORITHM:
			{
				...
				tcc_cipher_set_opmode(stAlgorithm.uOperationMode, stAlgorithm.uDmaMode);

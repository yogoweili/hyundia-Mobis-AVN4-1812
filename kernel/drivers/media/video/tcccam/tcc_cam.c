...
#define FEATURE_PREVIEW_RATIO_720P // modify to 720p preview that mismatch ratio

#define FEATURE_ANDROID_ICS

static int debug	   = 0;
#define dprintk(msg...)	if (debug) { printk(KERN_INFO "Tcc_cam: " msg); }
...
extern int check_camera_interrupt;
extern int clk_check(struct clk *clk);
...
extern void datv_get_preview_size(int *width, int *height);
extern void datv_get_capture_size(int *width, int *height);
#endif
#endif
...
static void cif_data_init(void *priv)
{
	...
	|| defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
...
static irqreturn_t isp_cam_isr1(int irq, void *client_data/*, struct pt_regs *regs*/)
{
	...
	if(ISP_CheckInterrupt()) {
		ISP_ClearInterrupt();
		dprintk ("isp_cam_isr1: Collision Detected!!!! \n");
		return IRQ_HANDLED;
	}
#endif
			...
			if(skip_frm == 0 && !frame_lock) {
				...
				if(next_buf != &data->list && curr_buf != next_buf && (chkpnt==1)) {
					...
					if(prev_num != prev_buf->v4lbuf.index) {
						dprintk("Frame num mismatch :: true num  :: %d \n", prev_num);
						dprintk("Frame num mismatch :: false num :: %d \n", prev_buf->v4lbuf.index);
...
static irqreturn_t isp_cam_isr2(int irq, void *client_data/*, struct pt_regs *regs*/)
{
	...
	if ( ISP_CheckInterrupt() )
	{
		dprintk ("isp_cam_isr2: Collision Detected!!!!======================================== \n");
	}
#endif
	ISP_EventHandler3();
		...
		if(pWDMABase->uIRQSTS.nREG & VIOC_WDMA_IREQ_EOFF_MASK) {
			...
			if(skip_frm == 0 && !frame_lock) {
				...
				if(next_buf != &data->list && prev_buf != next_buf) { // exception process!!
					...
					prev_buf->v4lbuf.flags &= ~V4L2_BUF_FLAG_QUEUED;
					prev_buf->v4lbuf.flags |= V4L2_BUF_FLAG_DONE;
				//20141209 ysseung   modify to preview quality of viqe.
				#if 0 // defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150)  || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
					#if !defined(FEATURE_HW_COLOR_EFFECT)
					...
					#endif
				#else	// 30 frame
					if (hardware_data.cam_info != DAUDIO_CAMERA_CMMB) {//GT system start IM002A-375
				...
				if (check_camera_interrupt == 0) check_camera_interrupt = 1;
	...
	if(data->cif_cfg.oper_mode == OPER_CAPTURE) { // capture operation.
		if(pWDMABase->uIRQSTS.nREG & VIOC_WDMA_IREQ_EOFR_MASK) {
			if(skip_frm == 0 && !frame_lock) {
				...
				if(skip_frm > 0) {
					dprintk("capture frame skip. skip count is %d. \n", skip_frm);
					skip_frm--;
				} else {
					skip_frm = 0;
				}
...
void cif_scaler_set(void *priv, enum cifoper_mode mode)
{
	...
	// todo: 		
			...
			|| defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_RDA5888)\
			|| defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
			if(mode == OPER_CAPTURE)
			{
...
void cif_scaler_calc(void)
{
	...
	unsigned int off_x = 0, off_y = 0, width = 0, height = 0;
		...
		if(mode == OPER_CAPTURE) {
			...
			datv_get_preview_size(&PRV_W, &PRV_H);
			datv_get_capture_size(&CAP_W, &CAP_H);
			#endif
...
void cif_global_reset(void)
{
	...
	if(data->cif_cfg.oper_mode == OPER_CAPTURE)
		...
		TDD_CIF_SetCtrl(SET_CIF_II_ALL, CIF_PWDN_DISABLE, CIF_BYPASS_SCALER_DISABLE,
							data->cif_cfg.polarity_pclk, FRAME_0, data->cif_cfg.fmt, CIF_656CONVERT_DISABLE, bEnableOfATV);
#elif defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
	bEnableOfATV = TRUE;
	#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
...
{
	BITCSET(pDDIConfig->CIFPORT.nREG, 0x00077777, (4 << 16) | (3 << 12) | (2 << 8) | (1 << 4) | nUsingPort);
}
...
int tccxxx_vioc_vin_wdma_set(VIOC_WDMA *pDMA_CH)
{
	...
	volatile PVIOC_IREQ_CONFIG pIREQConfig = (volatile PVIOC_IREQ_CONFIG)tcc_p2v(HwVIOC_IREQ);
	...
	#if defined(FEATURE_HW_COLOR_EFFECT)
		VIOC_WDMA_SetImageR2YEnable(pDMA_CH, ON);
		VIOC_WDMA_SetImageR2YMode(pDMA_CH, R2YMD_SDTV_LR);
	#endif
...
int tccxxx_vioc_scaler_set(VIOC_SC *pSC, VIOC_VIN *pVIN, VIOC_WMIX *pWMIX, uint plug_in)
{
...
#if defined(FEATURE_PREVIEW_RATIO_720P) // modify to 720p preview that mismatch ratio

 	// check to ratio 4:3
       if((data->cif_cfg.oper_mode == OPER_PREVIEW) && ((dw*3/4) != dh)) {
               preview_ratio = 1; 
			   dprintk("%s():  mismatch preview ratio. \n", __FUNCTION__); 
       } 
 
       if((data->cif_cfg.oper_mode == OPER_PREVIEW) && (dw > width)) { 
               width  = dw; 
			   height = width * 3 / 4; 
       }
...
#if (0) // 2013.03.26 swhwang, disable PlagOut for VGA Sensor Zoom in & out Scaling bug
	...
	#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
               if (hardware_data.cam_info == DAUDIO_CAMERA_CMMB) {     //for CMMB
	...
	dprintk("%s():  channel=%d, Dst[%dx%d], WMIX[%dx%d], plug_in path: %d \n",\
			__FUNCTION__, channel, dw, dh, mw, mh, plug_in);
	...
	VIOC_SC_SetBypass(pSC, OFF); 	// scaling mode.
		...
		VIOC_SC_SetOutPosition(pSC, (mw-dw), (mh-dh));
			...
			if(preview_ratio) { 
				...
				VIOC_SC_SetOutPosition(pSC, ((width-dw)/2), ((height-dh)/2)); 
			} else { 
				#if defined(CONFIG_ARCH_TCC893X)
...
int tccxxx_vioc_viqe_main(uint use_top_size)
{
	...
	VIOC_VIQE_SetDeintlMode((VIQE *)(pVIQEBase), VIOC_VIQE_DEINTL_MODE_3D); //20141209 ysseung   modify to preview quality of viqe.
	//GT system start IM002A-375
/*	if (hardware_data.cam_info == DAUDIO_CAMERA_CMMB) {
		VIOC_VIQE_SetDeintlMode((VIQE *)(pVIQEBase), VIOC_VIQE_DEINTL_MODE_3D); //20141209 ysseung   modify to preview quality of viqe.
	} else {
		VIOC_VIQE_SetDeintlMode((VIQE *)(pVIQEBase), VIOC_VIQE_DEINTL_MODE_2D); //20141209 ysseung   modify to preview quality of viqe.
	}*/
	//GT system end
	VIOC_VIQE_SetControlEnable((VIQE *)(pVIQEBase), cdf_lut_en, his_en, gamut_en, d3d_en, deintl_en);
...
#if defined(FEATURE_HW_COLOR_EFFECT)
int tccxxx_vioc_lut_set(void)
{
	VIOC_LUT *pLUT = (VIOC_LUT*)tcc_p2v(HwVIOC_LUT);

	VIOC_LUT_Plugin(pLUT, g_rgb_cmd.lut_number, 25/*VIOC_SC_WDMA_05*/);
	VIOC_LUT_Set_value(pLUT, g_rgb_cmd.lut_number, g_rgb_cmd.rgb_table);
	VIOC_LUT_Enable(pLUT, g_rgb_cmd.lut_number, TRUE);
...
static int tccxxx_jpeg_encode_method(void)
{
	struct TCCxxxCIF *data = (struct TCCxxxCIF *) &hardware_data;
...
int tccxxx_cif_cam_recovery(void)
{
	struct TCCxxxCIF *data = (struct TCCxxxCIF *) &hardware_data;
	...
	tccxxx_vioc_viqe_wmix_set(pWMIXBase, VIOC_VIQE_VIN_00, OFF);

	// set to SCALER
	tccxxx_vioc_scaler_set(pSCBase, pVINBase, pWMIXBase, VIOC_SC_WDMA_05);
...
int tccxxx_cif_cam_restart(struct v4l2_pix_format *pix, unsigned long xclk)
{
	...
	#endif // FEATURE_ANDROID_ICS
	dprintk ("prevSize:(%d,%d)=%x restart \n", data->cif_cfg.main_set.target_x, data->cif_cfg.main_set.target_y, \
											  data->cif_cfg.main_set.target_x*data->cif_cfg.main_set.target_y);
...
int tccxxx_cif_start_stream(void)
{
	...
	struct TCCxxxCIF *data = (struct TCCxxxCIF *) &hardware_data;
	unsigned int stride = 0, y_offset = 0, uv_offset = 0;
	struct clk *cm3_clk = clk_get(NULL, "cortex-m3");
	...
	ISP_SetPreview_Control(ON);
		...
		if (clk_check(cm3_clk)) { // case of running to early-camera.
			...
			ret = VIOC_CONFIG_PlugOut(VIOC_SC0);
			if (ret != VIOC_PATH_DISCONNECTED) {
				BITCSET(pVIOCConfig->uSOFTRESET.nREG[0], (1<<28), (1<<28)); // SCALER0 reset
				BITCSET(pVIOCConfig->uSOFTRESET.nREG[0], (1<<28), (0<<28)); // SCALER0 reset clear
			}
			
			ret = VIOC_CONFIG_PlugOut(VIOC_DEINTLS);
			if (ret != VIOC_PATH_DISCONNECTED) {
				BITCSET(pVIOCConfig->uSOFTRESET.nREG[1], (1<<17), (1<<17)); // DEINTLS reset
				BITCSET(pVIOCConfig->uSOFTRESET.nREG[1], (1<<17), (0<<17)); // DEINTLS reset clear
			}
		}
		clk_put(cm3_clk);

		#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
			dprintk("%s cam_id: %d\n", __func__, hardware_data.cam_info);
		...
		if (hardware_data.cam_info == DAUDIO_CAMERA_CMMB)
			BITCSET(pDDIConfig->CIFPORT.nREG, 0x00077777, ((4<<16)|(3<<12)|(1<<8)|(0<<4)|(2)));
		else
...
{	
	...
	#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
		VIOC_VIQE_SetControlEnable(pVIQEBase, OFF, OFF, OFF, OFF, OFF);
...
int tccxxx_cif_capture(int quality)
{
	...
	#endif
	{
		...
		do {
			...
			dprintk("MT9T113 Autofocus 0[0x%x] 1[0x%x] data_read = 0x%x \n",data_r[0], data_r[1], data_read);
...
int  tccxxx_cif_init(void)
{
	...
	const int cam_info = hardware_data.cam_info;
	...
	hardware_data.cam_info = cam_info;
	...
	if (buf->area == NULL) 
	{
		dprintk(KERN_ERR CAM_NAME ": cannot remap buffer\n");
		return ENODEV;
	}
	
#else // JPEG_ENCODE_WITH_CAPTURE
	dprintk("tccxxx_cif_init cam_open = %d, cam_close = %d\n", cam_open, cam_close);

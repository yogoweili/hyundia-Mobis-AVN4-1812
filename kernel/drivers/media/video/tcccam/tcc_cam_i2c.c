...
{
	int id = 0, ver = daudio_main_version();
	switch (ver)
	{
...
#if defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
	if (i2c_write(I2C_CAMERA, (unsigned char)get_cam_slave_id(), data, reg_bytes,
			data+reg_bytes, data_bytes) == i2c_err)
#else
	if (i2c_write(I2C_CAMERA, (unsigned char)SENSOR_I2C_ADDR, data, reg_bytes,
			data+reg_bytes, data_bytes) == i2c_err)
#endif
	{
...
#if defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
	if (i2c_read(I2C_CAMERA, (unsigned char)get_cam_slave_id(), data,
			reg_bytes, val, val_bytes) == i2c_err)
#else
	if (i2c_read(I2C_CAMERA, (unsigned char)SENSOR_I2C_ADDR, data,
			reg_bytes, val, val_bytes) == i2c_err)
#endif
	{

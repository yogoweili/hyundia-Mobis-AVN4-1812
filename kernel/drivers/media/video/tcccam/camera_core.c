...
#if defined(CONFIG_CMMB_EARLY_RESET)
...
static int debug	   = 0;
#define dprintk(msg...)	if(debug) { printk(KERN_INFO "Camera_core: " msg); }
...
int check_camera_interrupt = 1; //20140723 ysseung   check to camera interrupt
...
camera_core_overlay_callback(void *arg1, void *arg)
{
	...
	spin_lock_irqsave(&cam->overlay_lock, irqflags); // spin lock check « ø‰

	if (!cam->previewing || cam->overlay_cnt == 0) {
		spin_unlock_irqrestore(&cam->overlay_lock, irqflags); // spin lock check « ø‰
		return;
	}
...
int tcc_videobuf_querybuf(struct v4l2_buffer *buf)
{
	...
	if(data->cif_cfg.fmt == 0) //yuv420
		buf->length 				= PAGE_ALIGN(pix.width*pix.height*2);	//?????Óø° ?¬∞? ¡∂¡§		
	else
		buf->length 				= PAGE_ALIGN(pix.width*pix.height*3/2);	//?????Óø° ?¬∞? ¡∂¡§		
...
int tcc_videobuf_dqbuf(struct v4l2_buffer *buf, struct file *file )
{
	...
	if (data->cam_info == DAUDIO_CAMERA_CMMB) {
		interval = 500;
	} else {
		interval = 250; // 150;
	}
	...
	if(list_empty(&data->done_list)) {
		...
		if(wait_event_interruptible_timeout(data->frame_wait, data->wakeup_int == 1, msecs_to_jiffies(interval)) <= 0) {
			//if (!data->wakeup_int) { // with spin lock 
				printk(KERN_INFO "wait_event_interruptible_timeout %dms!!\n", interval);
				if(check_camera_interrupt == 1) check_camera_interrupt = 0;
				//tccxxx_cif_cam_recovery();
				return -EFAULT;
			//}
		}
...
int tcc_check_camera_interrupt(int arg)
{
	return check_camera_interrupt;
}
...
{
	int ret;
	PVIOC_WDMA pWDMA = (PVIOC_WDMA)tcc_p2v(HwVIOC_WDMA05);
	...
	// ioremap?? probe???? ?????? virtual address problem¿∏?? ?Ãµ?.!!
	if (tccxxx_cif_init()) 
	{
...
static long camera_core_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	switch (cmd) {
		...
		case VIDIOC_USER_GET_VIDEO:
			return sensor_if_get_video((int *)arg);

		case VIDIOC_USER_GET_VIDEO_NOISE:
			return sensor_if_check_video_noise((int *)arg);

		case VIDIOC_USER_RESET_VIDEO_DECODER:
			return sensor_if_reset_video_decoder((int *)arg);

		case VIDIOC_USER_SET_PATH_REAR_CAMERA:
		case VIDIOC_USER_SET_PATH_AUX_NTSC:
		case VIDIOC_USER_SET_PATH_AUX_PAL:
			return sensor_if_set_path(&cmd);

		case VIDIOC_USER_GET_VIDEO_FORMAT:
			return sensor_if_get_video_format((int *)arg);

		case VIDIOC_USER_SET_WRITE_I2C:
			return sensor_if_set_write_i2c((I2C_DATA_Type *)arg);

		case VIDIOC_USER_GET_READ_I2C:
			return sensor_if_get_read_i2c((int *)arg);

		case VIDIOC_USER_GET_RGEAR_STATUS:
			return sensor_if_get_rgear_status((int *)arg);

		#if defined(FEATURE_SW_COLOR_EFFECT)
			case VIDIOC_USER_SET_SW_LUT_EFFECT:
				return sensor_if_set_sw_lut_effect((TCC_CAM_SW_LUT_SET_Type *)arg);

			case VIDIOC_USER_RUN_SW_LUT_EFFECT:
				return sensor_if_run_sw_lut_effect((TCC_CAM_LUT_INFO_Type *)arg);
		#endif

		#if defined(FEATURE_HW_COLOR_EFFECT)
			case VIDIOC_USER_SET_HW_RGB_EFFECT:
				return sensor_if_set_hw_rgb_effect((TCC_CAM_HW_RGB_LUT_SET_Type *)arg);

			case VIDIOC_USER_SET_HW_YUV_EFFECT:
				return sensor_if_set_hw_yuv_effect((TCC_CAM_HW_YUV_LUT_SET_Type *)arg);
		#endif

		case VIDIOC_USER_GET_CAM_INT_STATUS:
			return tcc_check_camera_interrupt((int)arg);

#if defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
		case DAUDIO_CHECK_VIDEO_SIGNAL:
			return datv_check_video_signal(arg);

		case DAUDIO_DISPLAY_MUTE:
			datv_display_mute(arg);
			break;

		case DAUDIO_DISABLE_BT656:
			datv_close_cam();
			break;

		case DAUDIO_RESET_REARCAMERA:
			return sensor_if_reset_video_decoder((int *)arg);
#endif
...
#if defined(CONFIG_VIDEO_ATV_SENSOR_DAUDIO)
	if (file->f_flags & DAUDIO_CAMERA_IO_FLAG)
		return 0;
#endif
...
#if defined(CONFIG_CMMB_EARLY_RESET)
	gpio_set_value(TCC_GPF(5), 0);
	mdelay(10);
	gpio_set_value(TCC_GPF(5), 1);
	printk("[pigfish] Reset CMMB Module Clear\n");
#endif

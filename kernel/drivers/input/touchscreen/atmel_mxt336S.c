...
#define RESET_DELAY				28
	...
	#if (DEBUG_MXT_336S)
		#define VPRINTK(fmt, args...) printk(KERN_INFO "[cleinsoft mXT336S-AT] " fmt, ##args)
	#else
...
static void hw_reset_gpio(void)
{
	unsigned RESET_GPIO = HW_RESET_GPIO;

	int touch_scl = get_gpio_number(CTL_I2C_TOUCH_SCL);
	int touch_sda = get_gpio_number(CTL_I2C_TOUCH_SDA);
	int gpio_enable = 0;
	int gpio_ret = 0;
	...
	if (gpio_enable != -1) {
		...
		gpio_ret = tcc_gpio_config(touch_scl, GPIO_FN7 | GPIO_OUTPUT | GPIO_LOW);
		if (gpio_ret < 0)
			printk(KERN_ERR "[%s:%d] gpio -> scl error value (0x%x) \r\n",__func__,__LINE__,gpio_ret);

        gpio_ret = tcc_gpio_config(touch_sda, GPIO_FN7 | GPIO_OUTPUT | GPIO_LOW);
		if (gpio_ret < 0)
			printk(KERN_ERR "[%s:%d] gpio -> sda rror value (0x%x) \r\n",__func__,__LINE__,gpio_ret);
	}
...
{
	int jig_det = 0;
	int gpio_jig_det = get_gpio_number(DET_TOUCH_JIG);
	if (gpio_jig_det < 0) {
		return -EFAULT;
	}
...
/**
  ...
  **/
static void gpio_func_enable(u8 enable)
{
    int gpio_i2s_scl = get_gpio_number(CTL_I2C_TOUCH_SCL);
    int gpio_i2s_sda = get_gpio_number(CTL_I2C_TOUCH_SDA);
...
static int _check_recalibration(struct mxt_data *mxt)
{
	...
	rc = check_touch_flag(mxt, &tch_ch, &atch_ch);
	if (!rc) {
...
static void mxt_check_touch_ic_timer_dwork(struct work_struct *work)
{
	...
	**/
	...
	if(error==0){ // no detect jig
		...
		{
			//exception
			printk(KERN_ERR "%s failed to read MXT_ADDR_INFO_BLOCK\n", __func__);
...
static int check_touch_flag(struct mxt_data *mxt, int *touch, int *anti_touch)
{
	...
	**/
		...
		&& (data_buffer[1] == MXT_MSG_T37_CMD_TOUCH_HEAD[1])))
	{
		...
		mxt_debug_info("[TSP] Waiting for diagnostic data to update, try %d\n", try_ctr);
	}
		...
		&& (data_buffer[1] == MXT_MSG_T37_CMD_TOUCH_HEAD[1]))
	{
		...
		mxt_debug_info("[TSP] Flags Counted channels: t:%d a:%d \n", tch_ch, atch_ch);
                dev_info(&mxt->client->dev, "tch_ch:%d, atch_ch:%d\n",tch_ch, atch_ch);
		/* send page up command so we can detect when data updates next time,
		 * page byte will sit at 1 until we next send F3 command */
		data_byte = 0x01;
...
static void check_chip_calibration(struct mxt_data *mxt)
{
	...
	**/
	...
	check_touch_flag(mxt, &tch_ch, &atch_ch);
		...
		if (mxt->cal_check_flag != 1) {
			mxt_debug_info("[TSP] check_chip_calibration just return!! finger_cnt = %d\n", finger_cnt);
			return;
		}
...
retry_i2c:
	...
	if (
		//((val <= 10) &&
		//(mxt->device_info.variant_id == MXT336S_FIRM_VER1_VARIANTID)) ||
		((mxt->device_info.variant_id == MXT336S_FIRM_VER1_VARIANTID)) ||
		//((val >= 21) &&
...
static int mxt_get_conf_version(struct mxt_data *mxt)
{
	...
	buff[0] = T38_USERDATA0;
	buff[1] = T38_USERDATA1;
	buff[2] = T38_USERDATA2;
	buff[3] = T38_USERDATA3;
	buff[4] = T38_USERDATA4;
	buff[5] = T38_USERDATA5;
	buff[6] = T38_USERDATA6;
	buff[7] = T38_USERDATA7;
	buff[8] = '\0';
...
static int mxt_write_init_registers(struct mxt_data *mxt)
{
	...
	VPRINTK("[%s] drv_version: %d, nvm_version: %d\n",
			__func__, drv_version, nvm_version);

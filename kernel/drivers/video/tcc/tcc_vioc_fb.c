...
struct sw_sync_timeline *timeline_fb, *timeline_ext;
int timeline_fb_max, timeline_ext_max;
struct work_struct fence_work, ext_fence_work;
static char ext_out_ret = 0;
#endif
...
int tccxxx_wmixer1_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
...
#if defined(CONFIG_SYNC)
static void fence_handler(struct work_struct *work)
{
	...
	tca_fb_pan_display(NULL, NULL);
#endif
	sw_sync_timeline_inc(timeline_fb, 1);//printk(" =================> sw_sync_timeline_inc %d \n", timeline_fb_max);
}

static void ext_fence_handler(struct work_struct *work)
{
	TCC_OUTPUT_FB_UpdateSync(Output_SelectMode);
	sw_sync_timeline_inc(timeline_ext, 1);//printk(" =================> sw_sync_timeline_inc %d \n", timeline_ext_max);
}
#endif
	...
	if(tccvid_vsync.outputMode == Output_SelectMode )
	{
		...
		switch(Output_SelectMode)
		{
			...
			case TCC_OUTPUT_HDMI:
				...
				{
#ifdef MVC_PROCESS
					if( pNextImage->MVCframeView == 1 && pNextImage->MVC_Base_addr0){
...
int tcc_video_lastframe_move(struct tcc_lcdc_image_update* inFframeInfo, WMIXER_INFO_TYPE* WmixerInfo, unsigned int target_addr, unsigned int target_format)
{
	int ret = 0;
	...
	tccxxx_wmixer1_open((struct inode *)&lastframe_wm_inode, (struct file *)&lastframe_wm_flip);
	ret = tccxxx_wmixer1_ioctl((struct file *)&lastframe_wm_flip, TCC_WMIXER_IOCTRL_KERNEL, WmixerInfo);
	tccxxx_wmixer1_release((struct inode *)&lastframe_wm_inode, (struct file *)&lastframe_wm_flip);

	printk("### tcc_video_lastframe_move pre-processing(%dx%d - 0x%x) \n", inFframeInfo->Frame_width, inFframeInfo->Frame_height, inFframeInfo->addr0);
...
int tcc_video_last_frame(int res_changed)
{
	...
	if(Output_SelectMode == TCC_OUTPUT_HDMI ||Output_SelectMode== TCC_OUTPUT_COMPOSITE || Output_SelectMode == TCC_OUTPUT_COMPONENT )
	{
		...
		//tcc_video_lastframe_move(&lastUpdated, &WmixerInfo, fb_lastframe_pbuf.base, TCC_LCDC_IMG_FMT_YUYV);
		...
		tccxxx_wmixer1_ioctl((struct file *)&lastframe_wm_alpha_flip, TCC_WMIXER_ALPHA_SCALING_KERNEL, &fbmixer);
		...
		ScalerInfo.src_fmt			= lastUpdated.fmt;
		...
		ScalerInfo.src_Yaddr		= (char*)lastUpdated.addr0;
		...
		tccxxx_scaler2_ioctl((struct file *)&lastframe_sc_flip, TCC_SCALER_IOCTRL_KERENL, &ScalerInfo);
	...
	sprintk("%s addr:0x%x Yoffset:%d \n", __func__, base_addr, var->yoffset);
	...
	#endif
		{
			...
			if(TCC_OUTPUT_FB_GetMVCStatus())
				tcc_output_ret = TCC_OUTPUT_FB_Update_3D(fbi->fb->var.xres, fbi->fb->var.yres, fbi->fb->var.bits_per_pixel, base_addr, Output_SelectMode);
			else
...
static int tccfb_ioctl(struct fb_info *info, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TCC_LCDC_SET_COLORENHANCE:
			{
				...
				VIOC_DISP_GetColorEnhancement(pDISP, &get_colorenhance.contrast, &get_colorenhance.brightness, &get_colorenhance.hue);
		...
		case TCC_LCDC_HDMI_START:
			...
			#ifdef CONFIG_EXTEND_DISPLAY_DELAY
			ExtDispStruct.displaydata = (unsigned long)&last_sc_info;
	...
	case TCC_LCDC_VIDEO_PUSH_VSYNC:
		...
		//if(Output_SelectMode == TCC_OUTPUT_NONE || Output_SelectMode == TCC_OUTPUT_HDMI || Output_SelectMode == TCC_OUTPUT_COMPONENT || Output_SelectMode == TCC_OUTPUT_COMPOSITE)
		{
			...
			if(tccvid_vsync.deinterlace_mode < 0)
			{
				...
				if((tccvid_vsync.outputMode == TCC_OUTPUT_COMPOSITE) && (tccvid_vsync.deinterlace_mode == 1) && (tccvid_vsync.interlace_output == 1) && (input_image.Frame_width == input_image.Image_width))
				{
				...
				else
				{
					...
					switch(Output_SelectMode)
					{
						...
						case TCC_OUTPUT_HDMI:
							...
							#ifdef MVC_PROCESS
							if( input_image.MVCframeView == 1 && input_image.MVC_Base_addr0){
...
TCC_VSYNC_PUSH_ERROR:
	...
	case TCC_LCD_FBIOPUT_VSCREENINFO:
		{
...
#if defined(CONFIG_SYNC)
			{
				...
				timeline_fb_max++;
				fd = get_unused_fd();
				pt = sw_sync_pt_create(timeline_fb, timeline_fb_max);
				fence = sync_fence_create("display", pt);
				sync_fence_install(fence, fd);
				var->reserved[3] = fd;
				var->reserved[4] = 0xf;
			}
			schedule_work(&fence_work);
#endif
		}
		break;
	...
	case TCC_HDMI_FBIOPUT_VSCREENINFO:
		{
	...
	#if defined(CONFIG_SYNC)
			{
				...
				timeline_ext_max++;
				fd = get_unused_fd();
				pt = sw_sync_pt_create(timeline_ext, timeline_ext_max);
	...
	#if defined(CONFIG_SYNC)
			schedule_work(&ext_fence_work);
			if (copy_to_user(arg, (void*)&sc_info, sizeof(external_fbioput_vscreeninfo)))
				return -EFAULT;
	#endif
...
static int tccfb_probe(struct platform_device *pdev)
{
	...
	for (plane = 0; plane < CONFIG_FB_TCC_DEVS; plane++)
	{
		...
		memset(info->map_cpu, 0x00, fbinfo->var.xres *fbinfo->var.yres_virtual * 4 );
...
#if defined(CONFIG_SYNC)
	timeline_fb = sw_sync_timeline_create("tccfb");
	timeline_ext = sw_sync_timeline_create("tccfb-ext");
	timeline_fb_max = timeline_ext_max = 0;

	INIT_WORK(&fence_work, fence_handler);
	INIT_WORK(&ext_fence_work, ext_fence_handler);
#endif

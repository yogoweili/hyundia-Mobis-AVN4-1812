...
#if defined(CONFIG_ARCH_TCC893X)
	#if defined(CONFIG_CHIP_TCC8933S) || defined(CONFIG_CHIP_TCC8935S) || defined(CONFIG_CHIP_TCC8937S) || defined(CONFIG_TCC_OUTPUT_ATTACH)
		iSCType = VIOC_SC1;
	#else
		iSCType = VIOC_SC3;
	#endif
...
void TCC_HDMI_DISPLAY_UPDATE(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	if((!lcd_width) || (!lcd_height)){
		printk(" %s Error :: lcd_width %d, lcd_height %d hdmi_lcdc=%d\n", __func__,lcd_width, lcd_height,hdmi_lcdc);
		return;
	}
...
void TCC_HDMI_DISPLAY_UPDATE_3D(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	unsigned int lcd_width = 0, lcd_height = 0, lcd_h_pos = 0, lcd_w_pos = 0, iVBlank;

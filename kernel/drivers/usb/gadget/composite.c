...
static void reset_config(struct usb_composite_dev *cdev)
{
	...
	wake_lock_timeout(&usb_config_wake_lock, 1*HZ);
	DBG(cdev, "%s : usb reset config wake unlock --\n", __func__);
}
	...
	if (number) {
		list_for_each_entry(c, &cdev->configs, list) {
			if (c->bConfigurationValue == number) {
				...
				DBG(cdev, "%s : usb set config wake lock ++\n", __func__);
				result = 0;
				break;
			}
		}
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
   ...
   DBG(cdev, "%s: %d\n", __func__, w_index);
	...
	case USB_REQ_GET_DESCRIPTOR:
      DBG(cdev, "%s:USB_REQ_GET_DESCRIPTOR\n", __func__);
	...
	case USB_REQ_SET_CONFIGURATION:
      DBG(cdev, "%s:USB_REQ_SET_CONFIGURATION\n", __func__);
	...
	case USB_REQ_GET_CONFIGURATION:
      DBG(cdev, "%s:USB_REQ_GET_CONFIGURATION\n", __func__);
		if (ctrl->bRequestType != USB_DIR_IN)
			goto unknown;
		if (cdev->config)
	...
	case USB_REQ_SET_INTERFACE:
      DBG(cdev, "%s:USB_REQ_SET_INTERFACE\n", __func__);
	...
	case USB_REQ_GET_INTERFACE:
      DBG(cdev, "%s:USB_REQ_GET_INTERFACE\n", __func__);

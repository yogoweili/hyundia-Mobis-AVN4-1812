...
#if defined(CONFIG_USB_SERIAL_PIO_LGIT_MODEM)
static __u16 vendor  = 0x1D74;
static __u16 product = 0x2110;
#elif defined(CONFIG_USB_SERIAL_KT_LGIT_MODEM)
static __u16 vendor  = 0x1D74;
static __u16 product = 0x2202;
#else
			       ...
			       const struct usb_device_id *id)
{
	...
	if(hcd->driver->description != NULL &&
			hcd->driver->description[0] == 'x'){
		dbg("xhci not support usbserial_generic\n");
		return -EPERM;
	}
...
int usb_serial_generic_register(int _debug)
{
	...
	generic_device_ids[1].idProduct = product;

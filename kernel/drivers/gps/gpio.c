 ...
 **/
	...
	{
		gpio_set_value(TCC_GPE(17), 0); // tcc892x : gpio_e(14)
	}
#else
...
{  
	...
	else if(machine_is_tcc8800() || machine_is_tcc8920()) 
	{
		...
		tcc_gpio_config(TCC_GPEXT1(6), GPIO_FN(0));
		gpio_request(TCC_GPEXT1(6), "GPIO_PWREN");
 ...
 **/
	#if defined(CONFIG_DAUDIO)
		printk("%s:%s TODO: gpio port mapping", __FILE__, __FUNCTION__);
		tcc_gpio_config(TCC_GPA(4), GPIO_FN(0));    /// valky@cleinsoft - change port and function value.
		gpio_request(TCC_GPA(4), "GPIO_PWREN");
		gpio_direction_output(TCC_GPA(4), 0);    // GPS Power On
	#else
		tcc_gpio_config(TCC_GPEXT1(6), GPIO_FN(0));
		gpio_request(TCC_GPEXT1(6), "GPIO_PWREN");
		gpio_direction_output(TCC_GPEXT1(6), 0);    // GPS Power On

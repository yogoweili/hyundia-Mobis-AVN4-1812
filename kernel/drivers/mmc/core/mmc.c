...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if(ext_csd[EXT_CSD_REV] > 6) card->ext_csd.rev=5;
        else card->ext_csd.rev = ext_csd[EXT_CSD_REV];
	...
	case EXT_CSD_CARD_TYPE_DDR_52 | EXT_CSD_CARD_TYPE_52 |
		...
		card->ext_csd.hs_max_dtr = 45000000;
...
static int mmc_resume(struct mmc_host *host)
{
	...
	if (do_hibernate_boot) {
		printk("[mmc] Start %s without oldcard data...\n", __func__);
		host->card = NULL;
		err = mmc_init_card(host, host->ocr, host->card);
	} else {
		printk("[mmc] Start %s with oldcard data...\n", __func__);
		err = mmc_init_card(host, host->ocr, host->card);
	}
#else
	err = mmc_init_card(host, host->ocr, host->card);

...
CLOG_INIT(CLOG_TAG_ORGANIZATION, CLOG_TAG_MMC, CLOG_LEVEL_MMC);
...
#if 0//defined(CONFIG_ARCH_TCC893X) //&& !defined(CONFIG_BROADCOM_WIFI) && !defined(CONFIG_WIFI_PWR_CTL)
...
**/
	...
	if (host->pdata->clock_onoff)
		host->pdata->clock_onoff(host->dev, host->id, 1);
#endif
...
static void tcc_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock != 0) {
		...
		CLOG(CLL_INFO, "[%s] ios->clock : %d, host->peri_clk: %d, clk_div : %d\n",
		    __func__, ios->clock, host->peri_clk, clk_div);
		if (ios->timing == MMC_TIMING_UHS_DDR50) {
			CLOG(CLL_INFO, "[tcc_mmc] Start DDR clock...\n");
...
static int get_sdr_mode(int caps)
{
	if (caps & MMC_CAP_UHS_SDR12) {
		CLOG(CLL_INFO, "[mmc] MMC_CAP_UHS_SDR12 is enabled.\n");
		return 0x00080000;
	}
	if (caps & MMC_CAP_UHS_SDR25) {
		CLOG(CLL_INFO, "[mmc] MMC_CAP_UHS_SDR25 is enabled.\n");
		return 0x00090000;
	}
	if (caps & MMC_CAP_UHS_SDR50) {
		CLOG(CLL_INFO, "[mmc] MMC_CAP_UHS_SDR50 is enabled.\n");
		return 0x000A0000;
	}
	if (caps & MMC_CAP_UHS_DDR50) {
		 ...
		 */
		CLOG(CLL_INFO, "[mmc] MMC_CAP_UHS_DDR50 is enabled.\n");
		return 0x000C0000;
	}
	CLOG(CLL_INFO, "[mmc] Illegal MMC_CAP_UHS_XXX is set.");
		...
		int card_drv)
{
	...
	if (drv_caps & SD_DRIVER_TYPE_D) {
		CLOG(CLL_INFO, "[mmc] %s, SD_DRIVER_TYPED_D is supported.\n", __func__);
		return 0x3;
	}
	if (drv_caps & SD_DRIVER_TYPE_C) {
		CLOG(CLL_INFO, "[mmc] %s, SD_DRIVER_TYPED_C is supported.\n", __func__);
		return 0x2;
	}
	if (drv_caps & SD_DRIVER_TYPE_A) {
		CLOG(CLL_INFO, "[mmc] %s, SD_DRIVER_TYPED_A is supported.\n", __func__);
		return 0x1;
	}
	if (drv_caps & SD_DRIVER_TYPE_B) {
		CLOG(CLL_INFO, "[mmc] %s, SD_DRIVER_TYPED_B is supported.\n", __func__);
		return 0x0;
	}
	CLOG(CLL_INFO, "[mmc] %s, Illegal DRIVER_TYPE is set.\n", __func__);
...
#if defined(CONFIG_MMC_TCC_EMMC_CLOCK_CONTROL)
	if (host->pdata->clock_onoff)
		host->pdata->clock_onoff(host->dev, host->id, 0);
#endif
		...
		unsigned int clock_rate)
{
	...
	switch (host->pdata->slot % 4) {
		...
		case 1:
			 ...
			 **/
			sdctrl->SD1CMDDAT.nREG = 0x00000F0F;
...
extern struct mmc_host *tcc_mmc_sd;
#endif
static int tcc_mmc_probe(struct platform_device *pdev)
{
	...
	if (host->pdata->is_kpanic_card) {
		host->is_kpanic_card = host->pdata->is_kpanic_card;
		init_tcc_mmc_kpanic();
	}
#endif

#ifdef CONFIG_HIBERNATION
	if (host->pdata->is_external_sd) {
		tcc_mmc_sd = mmc;
	}
#endif
...
//	host->cd_irq = platform_get_irq(pdev,  1);
	...
	if(host->id==1) {
	#endif
            CLOG(CLL_INFO, "[tcc_mmc] Broadcom WiFi is enabled at id no.%d\n", host->id);
...
// FIXME: Should move?
	...
	device_create_file_clog(&pdev->dev);

	CLOG(CLL_INFO, "%s: SDHC%d init\n", DRIVER_NAME, host->id);
	return ret;

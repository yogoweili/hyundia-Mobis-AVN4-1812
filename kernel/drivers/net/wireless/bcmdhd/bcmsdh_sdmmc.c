...
//=============================================================     
	...
	struct sdio_func *sdio_func = sd->func[func]; // jaehyungkim 2015.01.06
...
//jaehyungkim@mobis.co.kr 2015.01.06
	...
	if (0 != lft_len) {
		/* Claim host controller */
		//sdio_claim_host(gInstance->func[func]);
		sdio_claim_host(sdio_func); // jaehyungkim 2015.01.06
        for (pnext = pkt; pnext; pnext = PKTNEXT(sd->osh, pnext)) {
...
txglomfail:
			...
			if ((write) && (!fifo))
				err_ret = sdio_memcpy_toio(
						//gInstance->func[func],
                        sdio_func, // jaehyungkim 2015.01.06						
						addr, buf, pkt_len);
			else if (write)
				err_ret = sdio_memcpy_toio(
                        //gInstance->func[func],
		...
		//sdio_release_host(gInstance->func[func]);
		sdio_release_host(sdio_func); // jaehyungkim 2015.01.06         
	}
#ifdef BCMSDIOH_TXGLOM
	if (localbuf)

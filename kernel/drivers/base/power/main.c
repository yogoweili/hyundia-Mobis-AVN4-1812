 ...
 */
#if defined(CONFIG_PM_VERBOSE_DPM_SUSPEND) || defined(CONFIG_HIBERNATION)
...
void dpm_resume(pm_message_t state)
{
	...
	if (do_hibernate_boot) {
		list_for_each_entry(dev, &dpm_suspended_list, power.entry) {
			if (!is_async(dev)) {
				...
				if (dev->driver != NULL && dev->driver->name != NULL
						&& (!strcmp(dev->driver->name, "tcc-i2c")
								|| !strcmp(dev->driver->name, "tcc-uart"))) {
					int error;
					...
					error = device_resume(dev, state, false);
					suspend_test_finish_dev(dev, TEST_SUSPEND_TIME_LEVEL1,
							"device_resume");
		...
		// I removed mdelay(100) in tcc_i2c_init fuction when boot with snapshot image, because insert below mdelay.
		mdelay(100);
	}
#endif
...
#if defined(CONFIG_PM_VERBOSE_DPM_SUSPEND)
			if (do_hibernate_boot && dev->driver != NULL

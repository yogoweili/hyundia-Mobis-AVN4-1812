...
void tcc_ehci_clkset(int on)
{
	if (machine_is_tcc8800st() || machine_is_tcc8800() || 
		...
		machine_is_tcc8920()  || machine_is_tcc8920st() || machine_is_tcc8930st() || machine_is_m805_892x() || 
...
{
  if (machine_is_tcc8800() || machine_is_tcc8920() || machine_is_m805_892x() || machine_is_tcc893x()){
...
#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_CHIP_TCC8935S)
   if (machine_is_tcc8920() || machine_is_tcc893x()) {
		int gpio_otg0_vbus_en;
 ...
 **/
		...
		//gpio_otg0_vbus_en = TCC_GPEXT2(13); //ȸ?ε? ????	
#endif

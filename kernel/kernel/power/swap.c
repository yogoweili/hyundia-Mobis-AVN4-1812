...
void get_hibernate_sig(char *sig) {
	if (sig != NULL) {
		memcpy(sig, HIBERNATE_SIG, QB_SIG_SIZE);
	}
}
#endif
...
//+[TCCQB] QB_kmsg_fix
	snapshot_block = swsusp_resume_block;
//-[TCCQB] QB_kmsg_fix
//
...
//+[TCCQB] QB SIG 20
	if (!memcmp("SWAP-SPACE",swsusp_header->sig + QB_SIG_SIZE-10, 10) ||
	    !memcmp("SWAPSPACE2",swsusp_header->sig + QB_SIG_SIZE-10, 10)) {
		memcpy(swsusp_header->orig_sig,swsusp_header->sig, QB_SIG_SIZE);
		memset(swsusp_header->sig, 0, sizeof(swsusp_header->sig));
		memcpy(swsusp_header->sig, HIBERNATE_SIG, sizeof(HIBERNATE_SIG));
//-[TCCQB]
//
#ifdef CONFIG_SNAPSHOT_BOOT
			local_irq_save(flag);
	...
	for (;;) {
		...
		*(size_t *)cmp = cmp_len;
		...
		for (off = 0; off < LZO_HEADER + cmp_len; off += PAGE_SIZE) {
			...
			if (ret) {
				printk(KERN_ERR "\x1b[1;31mPM: Not enough free swap space!!!\x1b[0m\n");
				goto out_finish;
			}
		}
	}
...
//		printk("<0> Checksum : cmp_len : %d\n", cmp_len);
        checksumResult = checksum(cmp+LZ4_HEADER, cmp_len); // Move after Lz4 Compressing, and Check Compressed Image.

		/*		Show CRC Values					*/
//		static int CRC_cont = 0;
//		printk("<0> Kernel CRC : [%4d]\t\tvalue : [%x]\n", CRC_cont++, checksumResult);
...
static int enough_swap(unsigned int nr_pages, unsigned int flags)
{
	...
	required = PAGES_FOR_IO + ((flags & SF_NOCOMPRESS_MODE) ?
		nr_pages : ((nr_pages * LZ4_CMP_PAGES) / LZ4_UNC_PAGES + 1))/2;
#else
	required = PAGES_FOR_IO + ((flags & SF_NOCOMPRESS_MODE) ?
...
#if defined(CONFIG_SWAP_PREDICTION)
	if (!enough_swap(pages, flags)) {
		printk(KERN_ERR "\x1b[1;31mPM: Not enough predict free swap\x1b[0m\n");
		error = -ENOSPC;
		goto out_finish;
	}
#endif
...
out_finish:
	tccwdt_qb_kickdog();    // Kick QuickBoot WatchDog
	if(error) 
		printk(KERN_ERR "\x1b[1;31mPM: Failed to Make QuickBoot Image!!!\x1b[0m\n");
	error = swap_writer_finish(&handle, flags, error);
	return error;
}
//+[TCCQB] QB_kmsg_fix
//remove swsusp_check_qb_sig( ) function.
//-[TCCQB] QB_kmsg_fix
//
/**
...
//+[TCCQB] QB_kmsg_fix
void swsusp_check_qb(void)
{
	int error = 0;

	memset(swsusp_header, 0, 4096);
	error = hib_bio_read_page(snapshot_block, swsusp_header, 1);
	printk("Re-check swsusp_heder sig[%s] error[%d]\n", swsusp_header->sig, error);
